...
- Endpoint: `GET /api/clients/<id>/giustificativo` (optional query params: `data` YYYY-MM-DD, `ora_inizio`/`ora_fine` HH:MM; defaults: today, now, now+1h)
...
- Gender and birth date are decoded from the client's codice fiscale (`decode_codice_fiscale` in `backend/app/utils.py`); birth place is not derivable and is left as a dotted line
