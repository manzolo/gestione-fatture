...
### Giustificativo di presenza (attendance certificate)
- Endpoint: `GET /api/clients/<id>/giustificativo` (optional query params: `data` YYYY-MM-DD, `ora_inizio`/`ora_fine` HH:MM; defaults: today, one hour ago → now, i.e. the session just ended)
- Same DOCX→PDF flow as invoices (docxtpl + Gotenberg), returns ZIP with both files
- Template: `backend/app/templates/giustificativo_template.docx`, generated by `scripts/generate_giustificativo_template.py` from the invoice template (reuses its letterhead: everything before the client block is kept, the invoice body is replaced). For the production letterhead with logo: `python3 scripts/generate_giustificativo_template.py --base custom_template/invoice_template.docx --out custom_template/giustificativo_template.docx`
- Gender is decoded from the client's codice fiscale (`decode_codice_fiscale` in `backend/app/utils.py`); birth date uses the optional `Cliente.data_nascita` field if set, otherwise falls back to the CF-decoded date; birth place comes from the optional `Cliente.luogo_nascita` field, dotted line if empty
- IMPORTANT: after adding an Alembic migration, regenerate the test fixture `backups/backup.sql` (restore it, run `alembic upgrade head` in the backend container, `pg_dump` back) — `make test-*` restores the dump without re-running migrations
- Frontend: "Giustificativo" button on each client card → proxy route `/download_giustificativo/<id>`
