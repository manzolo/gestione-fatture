...
- Gender and birth date are decoded from the client's codice fiscale (`decode_codice_fiscale` in `backend/app/utils.py`); birth place comes from the optional `Cliente.luogo_nascita` field, dotted line if empty
